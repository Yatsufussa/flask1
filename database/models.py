...
class PostComment(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('user_post.post_id'), nullable=False)
    post_id = db.Column(db.Integer, db.ForeignKey('users.user_id'), nullable=False)
    comment_text = db.Column(db.String, nullable=True)
    post_date = db.Column(db.DateTime)

    user_fk = db.relationship(User)
    photo_fk = db.relationship(Post)
